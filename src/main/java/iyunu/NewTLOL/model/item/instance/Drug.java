	...
	public boolean use(Role role, int num, Map<Integer, Cell> cellsMap) {
		if (this.way == 0 && (this.apply == 0 || this.apply == 1)) {
			...
			if (recover.equals(ERecover.definite)) {
				...
				recoverMp = (int) (role.getMpMax() * (mp / 10000f) * num);
			}

			if (role.getHp() + recoverHp >= role.getHpMax()) {
				role.setHp(role.getHpMax());
			} else {
				role.setHp(role.getHp() + recoverHp);
			}

			if (role.getMp() + recoverMp >= role.getMpMax()) {
				role.setMp(role.getMpMax());
			} else {
				role.setMp(role.getMp() + recoverMp);
			}
