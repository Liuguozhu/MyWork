...
public class 震 implements SkillSpecies {

	private 震() {
	}

	private static 震 instance = new 震();

	public static 震 instance() {
		return instance;
	}
	...
	public boolean effect(BattleCharacter self, Integer foeIndex, BattleInfo battleInfo, BaseSkill baseSkill) {
		...
		for (BattleCharacter foe : foes) {
			...
			float harm = BattleForm.harm(self.getPropertyBattle().getMattack(), foe.getPropertyBattle().getMdefence(), baseSkill.getPercent()) + baseSkill.getValue(); // 内功伤害值
			int finalHarm = BattleServer.harm(harm, defenderAmin, self, foe, 1, true);
			// 生命值计算
