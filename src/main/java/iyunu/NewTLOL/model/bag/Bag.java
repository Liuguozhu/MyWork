...
public class Bag {
	...
	private boolean warn = false; // 背包异常警告
	private Role role;
	private boolean hasStone = false;

	public static Comparator<Cell> comparator = new Comparator<Cell>() {
		@Override
			...
			if (cellItem != null && cellItem.getId() == item.getId() && cellItem.getIsDeal() == item.getIsDeal()) {
				...
				if (leftCapacity > 0) {
					if (left <= leftCapacity) {
						...
						// // ======任务======
						// if (role.getItems().containsKey(item.getId())) {
						// LogManager.logOut("收集物品：" + item.getName());
						// role.getItems().get(item.getId()).add(item.getId(),
						// num);
						// }
	...
	// /**
	// * 添加物品 合成物品 需要返回索引 集合，比如合成宝石，合成后背包添加了一个新宝石
	// *
	// * @param item
	// * 物品对象
	// * @param num
	...
	// * @return 为null就是失败
	// */
	// public Set<Integer> add_returnIndex(Item item, int num, Map<Integer,
	// Cell> map, EItemGet eItemGet) {
	//
	// Set<Integer> set = new HashSet<>();
	// if (item == null || num <= 0 || map == null) { // 非法参数
	// LogManager.exception("角色编号【" + role.getId() + "】角色名称【" + role.getNick() +
	// "】背包添加物品异常item=" + item + ",num=" + num);
	// return set;
	// }
	//
	// int max = item.getMax(); // 物品最大叠加数量
	...
	// Cell cell = cells[i];
	// Item cellItem = cell.getItem();
	// if (cellItem != null && cellItem.getId() == item.getId() &&
	// cellItem.getIsDeal() == item.getIsDeal()) {
	// int leftCapacity = cellItem.getMax() - cell.getNum();
	// if (leftCapacity > 0) {
	// if (left <= leftCapacity) {
	// cell.setNum(cell.getNum() + left);
	// map.put(cell.getIndex(), cell);
	// // 此处添加到值里，代表要返回的 新增的索引值集合
	// set.add(cell.getIndex());
	//
	// // ======收集任务======
	// TaskManager.addCollectionTaskItem(role, item, num);
	//
	// AwardServer.addItem(role, item, num); // 发送通知
	// if (isNotice(item, eItemGet)) {
	// String content = "恭喜玩家" + StringControl.grn(role.getNick()) + "通过" +
	// eItemGet.getName() + "获得了" + StringControl.color(item.getColor(),
	// item.getName()) + " x " + num;
	// BulletinManager.instance().addBulletinRock(content, 1);
	// LogManager.itemGet(role, item, num, eItemGet);
	// }
	...
	public boolean removeByIndex(int index, int num, Map<Integer, Cell> cellsMap, EItemCost itemCost) {
		if (index < 0 || index > Bag.MAX_CELLS) {
			return false;
		}
	 ...
	 * @return 背包对象
	 */
	public static Bag oldDecode(String str) {
		Bag bag = null;
		try {
			String[] strs = str.split("&");
			int size = Translate.stringToInt(strs[0]);
			size = size >= Bag.DEFAULT_CELLS ? size : Bag.DEFAULT_CELLS;
			bag = new Bag(size);
			...
			bag = new Bag(Bag.DEFAULT_CELLS); // 背包解码错误，则返回一个空背包
			bag.setWarn(true);
		}
		return bag;
	}
	 ...
	 * @return 背包对象
	 */
	public static Bag decode(String str) {
		Bag bag = null;

		if (str == null || "".equals(str)) {
			return new Bag(Bag.DEFAULT_CELLS); // 背包解码错误，则返回一个空背包
		}
	...
	public void sortBag() {
		...
		for (int i = 0; i < cells.length; i++) {
			for (int j = i + 1; j < cells.length; j++) {
				if (cells[i].getItem() != null && cells[j].getItem() != null && cells[i].getItem().getId() == cells[j].getItem().getId()
						&& cells[i].getItem().getIsDeal() == cells[j].getItem().getIsDeal()) {
