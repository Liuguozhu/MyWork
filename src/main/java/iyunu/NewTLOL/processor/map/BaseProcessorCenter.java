 ...
 * 
 */
public class BaseProcessorCenter extends Thread {
	...
	public void run() {
		while (process) {
			...
			Role role = MapManager.instance().getBaseQueue();
			if (role != null) {
				MapMessage.refreshNearbyBase(role);
			}
		...
		LogManager.info("【角色基本信息变化处理器】已关闭");
	}
}
