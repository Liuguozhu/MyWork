...
public class EmoMap1ProcessorCenter extends Thread {
	...
	public void shutdown() {
		process = false;
		// 删除怪
		BaseMap baseMap = MapManager.instance().getMapById(MiningJson.instance().getEmoMapInfoRes().getEmo1Map());
	...
	public void run() {
		...
		if (process) {
			ChatMessage.sendChat(new Chat(EChat.system, 0, "恶魔岛—万竹涧", StringControl.yel("恶魔降临，让我们快快乐乐的杀戮吧！")));
			EmoMapManager.instance().fresh1(1);
		}

		while (process) {
			long startTime = System.currentTimeMillis(), spent = 0;
			if (process) {
				EmoMapManager.instance().fresh1(0);
			}
			try {
