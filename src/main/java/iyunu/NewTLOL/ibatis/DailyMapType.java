...
public class DailyMapType implements TypeHandler {

	@Override
	public Object getResult(ResultSet rs, String parameter) throws SQLException {
		String v = rs.getString(parameter);
		HashMap<Integer, DailyModelRole> map = JsonTool.decode(v, new TypeReference<HashMap<Integer, DailyModelRole>>() {
		});
		if (map == null) {
			map = new HashMap<Integer, DailyModelRole>();
		}
		return map;
	}
