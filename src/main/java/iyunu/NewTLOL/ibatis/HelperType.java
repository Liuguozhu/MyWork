...
public class HelperType implements TypeHandler {

	@Override
	public Object getResult(ResultSet rs, String parameter) throws SQLException {
		String v = rs.getString(parameter);
		HashMap<EHelper, Integer> map = JsonTool.decode(v, new TypeReference<HashMap<EHelper, Integer>>() {
		});
		if (map == null) {
			map = new HashMap<EHelper, Integer>();
		}
		return map;
	}
